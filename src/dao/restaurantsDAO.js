...
export default class RestaurantsDAO {
  ...
  static async getRestaurants({
    ...
    try {
      // Use Sequelize's query building capabilities for better security and simplicity
      const whereClause = {};

  if (filters.name) {
    // Case-insensitive search for 'name' using 'LIKE'
    whereClause.name = {
  ...
  if (filters.zipcode) {
    whereClause.postalcode = filters.zipcode;
      ...
      return { restaurantsList: rows, totalNumRestaurants: count };
    } catch (e) {
      console.error(`Unable to get restaurants, ${e}`);
      return { restaurantsList: [], totalNumRestaurants: 0 };
    }
